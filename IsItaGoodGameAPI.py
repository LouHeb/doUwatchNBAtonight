...
def GameName(g):
    ...
    Tm1 = g[:3]
    Tm2 = g[-3:]
    if Tm1 in LogoTeams and Tm2 in LogoTeams:
...
for Index in range(0,len(df[0]['GAME_ID'])):
    ...
    overtime = OverTime(OverallTimer,ScoreMargin)
    lanote = Stars(OverallTimer,ScoreMargin)



#    file.write(Date+' '+Matchup+' '+str(lanote)+'\n')
    file.write('<tr><td style="text-align:center">'+DateEnLettre(Date)+'</td><td style="text-align:center">'+GameName(Matchup)+'</td><td style="text-align:center">'+NoteHtml[lanote]+'</td></tr>\n')
